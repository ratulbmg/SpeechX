...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // SpeechX patch: this used to call `keyboard_state.create_string_for_key`
        // (-> `TISCopyCurrentKeyboardInputSource`/`TISGetInputSourceProperty`) on
        // every KeyPress, including modifier keys converted from FlagsChanged.
        // Those are Carbon/HIToolbox Text Input Source Manager calls, and TSM
        // enforces via `dispatch_assert_queue` that they only ever run on the
        // main thread — but this runs inside the CGEventTap callback, on
        // whichever thread called `listen()`. SpeechX deliberately runs that
        // listener on a dedicated background thread (its own main thread is
        // owned by the Tauri/AppKit run loop), so every keypress crashed the
        // app with SIGTRAP the moment it hit a real macOS-launched .app bundle.
        // `Event.name` (the resulting typed-character string) isn't read
        // anywhere in SpeechX — only `event_type` (which key, press or
        // release) is — so the fix is to just stop computing it.
